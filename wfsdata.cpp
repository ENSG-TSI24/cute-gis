...
void WFSData::open()
{
    ...
    m_dataset = static_cast<GDALDataset*>(
        GDALOpenEx("http://demo.mapserver.org/cgi-bin/wfs?SERVICE=WFS&VERSION=1.0.0&REQUEST=getFeatures&TYPENAME=cities",
...
void WFSData::getData()
{
    ...
    OGRLayer* layer = m_dataset->GetLayerByName("cities");
    if (layer == nullptr)
    {
        std::cout << "Layer 'cities' not found." << std::endl;
        return;
    }
